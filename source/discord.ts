...
const client = new DiscordJS.Client({
  intents: [
    //specify the intentiens of our bot, what he want to do in discord
    ...
    guild?.members
      ...
      .then((data: any) => {
        data.forEach((element: any) => {
          console.log(
            `user tag: ${element.user.tag}\tisBot: ${element.user.bot}`
          );
        });
      })
      .catch((err: any) => {
...
export const sendMessageParticipate = async (user: string) => {
  new Promise<void>(async (resolve, reject) => {
    try {
      ...
      const attachment = new DiscordJS.MessageAttachment(
        "./source/assets/images/legend.jpg"
      );
      const embed = new DiscordJS.MessageEmbed()
        .setDescription(`See you for the raffle <@${discordUser.id}> !`)
        .setTitle(`Good luck getting on the Legendary raffle of Meg4mint 👊`)
        .setColor(0xffdd00)
        // .setImage(discordUser.displayAvatarURL({ format: 'png' })) //display user Avatar
        .setImage("attachment://legend.jpg");
